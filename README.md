...
<td>An over-long prompt to an OLMoE or Inkling model came back as <b>HTTP 500 instead of <code>context_length_exceeded</code></b>, so a client that compacts its conversation on that error never got the chance — and three more malformed requests answered <b>"the engine failed"</b>, a 500 that OpenAI SDKs retry, rather than "bad request": a non-object tool argument and a non-object forced-tool choice, because the prompt was rendered before the validator ran, and a lone surrogate from a piped CJK console. The tuner swept GPU settings on engines that <b>never read them</b>, reporting measurement noise as "+70.7%, accepted", and <code>plan</code> and <code>doctor</code> went on recommending those settings to every model; the GPU DLL the launcher demanded was <b>not the one <code>doctor</code> reported as present</b>. And on a Korean or Japanese Windows, verifying a <b>valid</b> shard with its output redirected <b>exited 1 with an empty stdout</b>: the verifier's note carried an em dash the code page cannot encode, and so did the error it fell back to.</td>
</tr>
