...
<td>An over-long prompt to an OLMoE or Inkling model came back as <b>HTTP 500 instead of <code>context_length_exceeded</code></b>, so a client that compacts its conversation on that error never got the chance — and two more malformed requests answered <b>"the engine failed"</b>, a 500 that OpenAI SDKs retry, rather than "bad request": a non-object forced-tool choice, because the prompt was rendered before the validator ran, and a lone surrogate from a piped CJK console. In review: the same 500 for a non-object tool argument. The tuner swept GPU settings on engines that <b>never read them</b>, reporting measurement noise as "+70.7%, accepted", and <code>plan</code> and <code>doctor</code> went on recommending those settings to every model; the GPU DLL the launcher demanded was <b>not the one <code>doctor</code> reported as present</b>. And on a Korean or Japanese Windows, verifying a <b>valid</b> shard with its output redirected <b>exited 1 with an empty stdout</b>: the verifier's note carried an em dash the code page cannot encode, and so did the error it fell back to.</td>
</tr>
...
<sub>Also in review: <a href="https://github.com/huggingface/skills">huggingface/skills</a> (a cost estimator that read <code>7B</code> out of <code>Qwen2.5-72B</code> because it matched the shortest size string first, and a dataset inspector that reported a row count the endpoint it asked never returns) &middot; <a href="https://github.com/alphaXiv/OpenResearch">alphaXiv/OpenResearch</a> (<code>orx paper</code> kept only the last path segment of an old-style arXiv id, so <code>hep-th/9711200</code> became <code>9711200</code> and 404'd, and that is exactly the id <code>orx discover</code> returns for pre-2007 papers) &middot; <a href="https://github.com/unslothai/unsloth">unslothai/unsloth</a> (Studio listed Bonsai's DSpark speculative-decoding drafter files as ordinary quants, so picking the smallest, "Q4_1", loaded the drafter as the model).</sub>
